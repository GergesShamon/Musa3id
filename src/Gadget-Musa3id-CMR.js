...
$.when(mw.loader.using(["mediawiki.user", "oojs-ui-core", "oojs-ui-windows", ]), $.ready).then(function() {
	...
	var api = new mw.Api();
	var SectionName, SectionLink, SectionPath, sectionNumber, MentorResponse;
	var CRButton = new OO.ui.ButtonWidget({
		framed: false,
		label: '[Hazır Cevap]',
		classes: ['Musa3id-canned-response-button']
	});
	$('.mw-editsection').append(CRButton.$element);
	$(".Musa3id-canned-response-button").click(function() {
		SectionName = $(this).parent().parent()[0].innerText.replace('[kaynağı değiştir]', '').replace('[Hazır Cevap]', '').replace('[abone ol]', '').trim();
		...
		sectionNumber = SectionLink.replace(/Kullanıcı_mesaj:[a-zA-Z0-9_.-]+&section=/gi, "").replace(/&veaction=editsource/gi, "").replace(/&action=edit&section=/gi, "");
		cmrDialog();
	});
	...
	function cmrDialog() {
		...
		CannedResponseDialog.static.title = 'Musa3id (Beta) - Danışman';
		...
		CannedResponseDialog.prototype.initialize = function() {
			...
			this.content.$element.append(HeaderBar.$element, '<br><hr><br>', dropdown.$element, '<div class="Musa3id-mentor-response-preview-area"></div>');
			...
			dropdown.getMenu().on('choose', function(menuOption) {
				...
				api.get({
					...
					$('.Musa3id-mentor-response-preview-area').html(data.parse.text['*']);
				});
			});
		};
